...
public class LoadOption {
    ...
    // init leurre
    private HandlerBox box_leurre;

    // constructor
    public LoadOption (Context context) {
        this.context = context;
        main = getMain(context);
        box_leurre = new HandlerBox(context);
    }
    ...
    public void getOption (String server) {
        new Option().execute(server);
    }

    @SuppressLint("StaticFieldLeak")
    class Option extends AsyncTask<String, String, Boolean> {

        private DataLocal local = DataLocal.get(context);
